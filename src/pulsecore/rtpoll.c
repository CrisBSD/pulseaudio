...
static int io_event_work_cb(pa_rtpoll_item *item) {
    ...
    event = pa_rtpoll_item_get_work_userdata(item);
...
static pa_io_event* io_event_new(pa_mainloop_api *api, int fd, pa_io_event_flags_t events,
pa_io_event_cb_t callback,
                                 void *userdata) {
    pa_rtpoll *rtpoll;
    pa_io_event *event;
    ...
    pa_rtpoll_item_set_work_callback(event->rtpoll_item, io_event_work_cb, event);
...
static pa_time_event* time_event_new(pa_mainloop_api *api, const struct timeval *tv,
pa_time_event_cb_t callback,
                                     void *userdata) {
    pa_rtpoll *rtpoll;
    pa_time_event *event;
...
static void defer_event_set_destroy(pa_defer_event *event, pa_defer_event_destroy_cb_t
callback) {
    pa_assert(event);
